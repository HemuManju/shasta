...
class UaVPDController(object):
    def __init__(
        ...
        self.Kp_roll, self.Kp_pitch, self.Kp_yaw = Kp_rot[0], Kp_rot[1], Kp_rot[2]
    ...
    def compute_torque(
        ...
        des_x_acc, des_y_acc, des_z_acc = desired_acc[0], desired_acc[1], desired_acc[2]
        ...
        thrust = self.mass * (
            ...
            + self.Kd_z * (des_z_vel - current_z_vel)
        )

        # Calculate torques
        roll_torque = self.Kp_roll * (
            ((des_x_acc * sin(self.des_yaw) - des_y_acc * cos(self.des_yaw)) / self.g)
            - roll
        )
        pitch_torque = self.Kp_pitch * (
            ((des_x_acc * cos(self.des_yaw) - des_y_acc * sin(self.des_yaw)) / self.g)
            - pitch
        )
