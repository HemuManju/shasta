...
class InteractionManager(object):
    ...
    def update_action(self, blue_action, red_action):
        ...
        distance = np.linalg.norm(
            blue_action['centroid_pos'] - red_action['centroid_pos']
        )
    ...
    def action_lookup_string(self, key):
        ...
        action = 'action_manager.' + vehicle_type + '_platoons' + str([key]) + '.action'
        return action

    def update_actions(self, blue_team, red_team):
        # Check the closeness (this function might change)
        blue_team_pos, red_team_pos = self.get_team_position(blue_team, red_team)
        with_in_perimeter = check_perimeter(blue_team_pos, red_team_pos, self.config)
