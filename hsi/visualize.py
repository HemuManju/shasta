...
def plot_cpu_memory_profile(config):
    ...
    temp = None
    for i in range(5):
        read_path = config['log_path'] + 'cpu_memory_profile_exp_' + str(i + 1) + '.csv'
        temp = pd.read_csv(read_path)
        df_temp = pd.concat((df_temp, temp), ignore_index=True)
        values.append(temp[['cpu', 'memory', 'memory_percent', 'speed_up']].values)
    ...
    # Main dataframe
    df[['World size', 'No. UAV + UGV', 'Headless']] = temp[
        ['world_size', 'uav + ugv', 'headless']
    ]
    df[['CPU Usage (%)', 'Memory Usage (MB)', 'Memory Usage (%)', 'Speedup']] = np.mean(
        values, axis=-1
    )
    df[
        [
    ...
    for subject in subjects:
        for session in sessions:
            ...
            for i, stream in enumerate(streams):
                offset[stream].append(
                    temp[stream]['time_offsets'] - np.mean(temp[stream]['time_offsets'])
                )
